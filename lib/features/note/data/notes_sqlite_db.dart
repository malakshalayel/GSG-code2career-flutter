...
class NotesSqliteDb {
  NotesSqliteDb._();
  static final NotesSqliteDb instance = NotesSqliteDb._();
  factory NotesSqliteDb() => instance;
  ...
  static updateNoteFromDb(NoteModel note) async {
    int result = await _database.rawUpdate(
      'UPDATE $tableName SET $titleColumn] = "${note.title}", $contentColumn = "${note.content}" WHERE $idColumn = ${note.id}',
    );
  }
}
